...
function UpdateSection({selectedCripto, cryptoList, setCryptoList, setSelectedCripto}){
  ...
  <button className="main-detail__button"  onClick={()=>{
    clearInterval(intervalId)
    setAutoUpdate(!autoUpdate)}} style={autoUpdate? {display:"block"}:{display:"none"}}>Pause update</button>

<button className="main-detail__button" onClick={()=>{
    setIntervalId(setInterval(down, 1000))
    setAutoUpdate(!autoUpdate)}} style={autoUpdate? {display:"none"}:{display:"block"}}>Resume update</button>
